...
The charm reads five credential fields from the S3 relation via
`_s3_creds()` in `src/charm.py:657-665`: `bucket`, `endpoint`, `region`,
...
When all five credentials are present, `build_s3_env()` in
`src/pebble.py:348-366` injects the following environment variables into the
n8n container:
...
`"binary data: s3 (storage mount idle)"` (set at `src/charm.py:579-580`),
indicating that the filesystem mount is present but unused.
...
On `credentials_gone` the charm re-evaluates binary-data mode. If the
`binary-data` storage mount is attached, the charm falls back to filesystem
mode. If the mount is absent, the charm falls back to in-DB mode
(`src/charm.py:535-540`). No data migration is performed; files written to S3
while the relation was active remain in the bucket but will not be accessible
to n8n after the relation is removed.
